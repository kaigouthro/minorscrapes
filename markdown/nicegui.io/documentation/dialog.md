...
Awaitable dialogReplacing content
...
whether the dialog should be opened on creation (default: False)
...
Dialogs can be awaited. Use the `submit` method to close the dialog and return a result. Canceling the dialog by clicking in the background or pressing the escape key yields `None`.
