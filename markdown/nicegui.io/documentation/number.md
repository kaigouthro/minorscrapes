...
ClearableNumber of decimal places
...
displayed name for the number input
...
the step size for the stepper buttons

prefix:

    

a prefix to prepend to the displayed value

suffix:

    

a suffix to append to the displayed value

format:

    

a string like "%.2f" to format the displayed value
...
dictionary of validation rules, e.g. {'Too large!': lambda value: value < 3}
...
You can specify the number of decimal places using the `precision` parameter. A negative value means decimal places before the dot. The rounding takes place when the input loses focus, when
sanitization parameters like min, max or precision change, or when `sanitize()` is called manually.
